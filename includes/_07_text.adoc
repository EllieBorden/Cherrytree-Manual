...
* *Find in All Nodes Contents* - Searches for a sequence of characters in the bodies of every node in the document.
...
* *Find in Nodes Names and Tags* - Searches for a sequence of characters in the every node title and tag of the document.
+
...
. (*Optional*) Select any of the *Search Options* that are applicable to your effort:
+
[[figure-7.11]]
.Figure 7.11 
{search-menu2}
...
** *Replace* - Replace the current instance of the of the searched term with the replacement. (Applicable only to link:#_search_and_replace[Search and Replace] features.)
** *Undo* - Undoes the more recent *Replace* execution.

. (*Optional*) *Time Filter* options will be available when searching across multiple nodes. Select any option(s) that correspond with your effort, and click the adjacent date(s) to edit its value:
+
[[figure-7.13]]
.Figure 7.13
{search-menu3}
...
* *Replace in Nodes Names and Tags* -
* *Replace Again* -
